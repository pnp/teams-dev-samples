...
namespace Calling.Meetings
{
    ...
    public class JoinInfo
    {
        ...
        public static (ChatInfo, MeetingInfo) ParseJoinURL(string joinURL)
        {
            ...
            var regex = new Regex(Helpers.Constants.MeetingUrlRegex);
            var match = regex.Match(decodedURL);
            if (!match.Success)
            {
                throw new ArgumentException($"{Helpers.Constants.JoinURLCannotBeParsedException} {joinURL}.", nameof(joinURL));
            }

            using var stream = new MemoryStream(Encoding.UTF8.GetBytes(match.Groups[Helpers.Constants.Context].Value));
            var ctxt = (Context)new DataContractJsonSerializer(typeof(Context)).ReadObject(stream);
            var chatInfo = new ChatInfo
            {
                ThreadId = match.Groups[Helpers.Constants.Thread].Value,
                MessageId = match.Groups[Helpers.Constants.Message].Value,
