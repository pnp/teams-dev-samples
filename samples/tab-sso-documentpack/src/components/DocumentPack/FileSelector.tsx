...
const OneDriveIcon = () => (
  <Icon
    style={{ color: "#006ac0", fontSize: "30px" }}
...
const FileSelector: React.FC<IFileSelectorProps> = (props) => {
  ...
  const updateFiles = async (incommingFiles: any) => {
    let mappedAcc: IUploadableFile[] = await Promise.all(
      incommingFiles.map(async (file: any) => {
  ...
  const handleUploadFile = async () => {
    setUploading(true);
    try {
      const teamsUserCredential =
        TeamsUserCredentialContext.getInstance().getCredential();
      ...
      let mappedRequest: any[] = await Promise.all(
        ...
        <div className="ms-Grid" dir="ltr">
          ...
          >
            <div className="ms-Grid-col ms-sm12">
              <Card
                type="inner"
                title="Select files to create document pack"
                extra={<OneDriveIcon />}
              >
                <Dropzone
                  ...
                  {!uploading && documentPack !== null && (
                    <Result
                      ...
                      extra={[
                        <>
                          <PrimaryButton
                            href={documentPack.WebUrl}
                            target="_blank"
                            title="View document pack"
                            text="View document pack"
                          />
                        </>,
                      ]}
                    />
